class GeoQuadrantes{  
	...
	findNextObjByCod(cod, filters, operations, values){
		let obj = this.findObjByCod(cod);
		let objs = this.removeObjByCode(this.findNextObjsByCod(cod, 1, filters, operations, values), cod);

		return this.whichIsCloser(obj[this.translate('lat')], obj[this.translate('long')], objs);
	}
	findNextObjsBylocation(lat, long, minCount = 1, filters, operations, values){
		...
		for(let i = 1; i < Math.sqrt(this.countSubQuads);i++){
			...
			objects = this.filterObjs(objects, filters, operations, values);
		//	console.log(objects, filters, operations, values, objectsx);
	...
	findNextObjsByCod(cod, minCount = 0, filters, operations, values){
		let obj = this.findObjByCod(cod);
		let objs = this.findNextObjsBylocation(obj[this.translate('lat')], obj[this.translate('long')], minCount, filters, operations, values);
